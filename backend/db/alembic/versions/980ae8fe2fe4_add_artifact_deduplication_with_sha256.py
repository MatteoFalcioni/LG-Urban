...
def upgrade() -> None:
    ...
    op.add_column('artifacts', sa.Column('sha256', sa.String(length=64), nullable=False))
    op.add_column('artifacts', sa.Column('filename', sa.String(length=512), nullable=False))
    ...
    op.add_column('artifacts', sa.Column('session_id', sa.String(length=128), nullable=True))
    op.add_column('artifacts', sa.Column('run_id', sa.String(length=128), nullable=True))
    op.add_column('artifacts', sa.Column('tool_call_id', sa.String(length=128), nullable=True))
    op.create_index(op.f('ix_artifacts_sha256'), 'artifacts', ['sha256'], unique=False)
    op.create_index('ix_artifacts_thread_created', 'artifacts', ['thread_id', 'created_at'], unique=False)
...
def downgrade() -> None:
    # ### commands auto generated by Alembic - please adjust! ###
    op.add_column('artifacts', sa.Column('blob', postgresql.BYTEA(), autoincrement=False, nullable=True))
    op.add_column('artifacts', sa.Column('uri', sa.VARCHAR(length=2048), autoincrement=False, nullable=True))
    op.add_column('artifacts', sa.Column('type', sa.VARCHAR(length=64), autoincrement=False, nullable=False))
